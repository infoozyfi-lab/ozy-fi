...
export function normalizeMenuBlob(raw: MenuData, locale: Locale): MenuBlob {
  ...
  //
  // Save-failed bug report (data-model gap) — a product's own base `price`
  // (set on its main edit form) IS its Normaali/smallest size; the Sizes
  // editor (components/admin/SizesEditor.tsx) is only for ADDITIONAL tiers
  // above that base, so an admin never has to re-enter "Normaali" as its
  // own row. Before this fix, that base price was never actually offered
  // as a selectable size once any real tier existed: `sizeOptionsByProduct`
  // only ever holds the tiers an admin explicitly added (e.g. Pannu/Perhe),
  // so a product with 2 admin-added tiers ended up with sizeOptions =
  // [Pannu, Perhe] only — the plain base price had no entry at all, and
  // context/StoreContext.tsx's openProduct() (which pre-selects
  // `sizeOptions[0]`, same convention as base/sauce/cheese/dip) would
  // default a customer straight into the CHEAPEST ADDED TIER, never the
  // product's real base price. Confirmed via a real runtime harness
  // (worker/test-data/save-failed-bug-verify.js) — not just reasoned about.
  //
  // Fix: whenever a product has any real tier at all, prepend a synthetic
  // "Normaali"/"Regular" entry (delta 0 — the base price itself) so the
  // customer-visible ladder is always [base, ...admin-added tiers], and the
  // default-selected (index 0) tier is always the true base price. This
  // synthetic entry behaves identically to a real DB-backed option for
  // every downstream consumer: lib/pricing.ts's findDelta / calcUnitPriceFromSelection
  // and StoreContext.tsx's calcUnitPrice both just `.find()` an option by
  // id within THIS product's own sizeOptions array — they never care
  // whether an entry came from D1 or was synthesized here.
  //
  // Deliberately NOT applied when a product has no real tiers at all — it
  // keeps using the exact same single-entry FALLBACK_OPTION (id: 'default')
  // every other unconfigured option kind falls back to, so
  // components/ProductPage.tsx's `hasRealSizeTiers` check (which
  // distinguishes a genuine admin-configured group from that fallback by
  // checking `length === 1 && id === 'default'`) keeps working exactly as
  // before — an unconfigured product still shows no size selector at all.
  //
  // The synthetic id (`size-base-<productId>`) can never collide with a
  // real option id: every real option id is either admin-typed or
  // auto-generated by the admin POST route as `slugify(label)-<timestamp>`
  // (app/api/admin/[table]/route.ts) — neither convention produces this
  // exact `size-base-` prefix.
  products.forEach((p) => {
