// Name of the httpOnly cookie that carries the admin token. Also still
// accepted as `Authorization: Bearer <token>` (see readToken below) so the
// ~27 existing admin fetch() calls that build that header manually keep
// working unchanged during the transition off sessionStorage.
...
// Cookie preferred, Authorization header as fallback — see ADMIN_COOKIE_NAME.
function readToken(request) {
  const fromCookie = readCookie(request, ADMIN_COOKIE_NAME);
