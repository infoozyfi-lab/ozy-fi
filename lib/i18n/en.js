...
const en = {
  ...
  footer: {
    ...
    rights: (year) => `© ${year} ozy.fi. All rights reserved.`,
  ...
  productPage: {
    ...
    addToOrder: (price) => `Add to order — ${price}`,
    addToBundle: 'Add to bundle',
    addToBundleExtra: (price) => `Add to bundle — +${price}`,
    largeUpchargeDetail: (price) => `Large (+${price})`,
  },
  ...
  bundleModal: {
    ...
    choose: 'Choose',
    remove: 'remove',
    chooseItemBtn: (label) => `+ Choose ${label}`,
    item: 'item',
    fillEverySlot: 'Fill every slot to continue',
    addBundleToOrder: (price) => `Add bundle to order — ${price}`,
  },
  ...
  checkout: {
    ...
    title: 'Your order',
    itemsCount: (n) => `${n} item${n !== 1 ? 's' : ''} in your order`,
    ...
    couponApplied: (code, amount) => `${code} applied — −${amount}`,
    couponRemove: 'Remove',
    ...
    continue: 'Continue',
    continueWithTotal: (total) => `Continue — ${total}`,
  ...
  confirm: {
    ...
    thanks: (name) => `Thanks${name ? `, ${name}` : ''}! Your order is on its way.`,
    eta: 'Estimated ready time: 25–35 minutes',
    couponApplied: (amount) => `Coupon applied: −${amount}`,
    codNote: (total) => (
  ...
  track: {
    ...
    placedAt: (date) => `Placed ${date}`,
    estimatedReadyBy: (time) => <>Estimated ready by <strong>{time}</strong></>,
    minutesLeftSoon: 'should be ready any moment now',
    minutesLeftOne: 'about 1 min left',
    minutesLeft: (n) => `about ${n} min left`,
    ...
    deliveringTo: (addr, method) => `Delivering to ${addr} · ${method}`,
