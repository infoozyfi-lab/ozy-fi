...
const fi = {
  ...
  bundleModal: {
    ...
    choose: 'Valitse',
    remove: 'poista',
    chooseItemBtn: (label: string) => `+ Valitse ${label}`,
    item: 'tuote',
    fillEverySlot: 'Täytä jokainen kohta jatkaaksesi',
    addBundleToOrder: (price: string) => `Lisää paketti tilaukseen — ${price}`,
  },
  ...
  checkout: {
    ...
    title: 'Tilauksesi',
    itemsCount: (n: number) => (n === 1 ? '1 tuote tilauksessasi' : `${n} tuotetta tilauksessasi`),
    ...
    postalCodePlaceholder: 'esim. 01600',
    ...
    errorPostalCode: 'Anna kelvollinen 5-numeroinen postinumero.',
    ...
    couponApplied: (code: string, amount: string) => `${code} käytössä — −${amount}`,
    couponRemove: 'Poista',
  ...
  track: {
    ...
    placedAt: (date: string) => `Tilattu ${date}`,
    estimatedReadyBy: (time: string) => <>Arvioitu valmis <strong>{time}</strong></>,
  ...
  legal: {
    lastUpdated: (date: string) => `Päivitetty viimeksi: ${date}`,
  },
