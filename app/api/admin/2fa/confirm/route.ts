...
export async function POST(request: Request) {
  ...
  // requireAdmin() just confirmed a valid session on this same request, so
  // this second getSession() call is for its data, not to re-check auth —
  // the assertion matches that existing guarantee (identical to the
  // implicit assumption this code already made pre-strict-mode).
  ...
  const staff = await env.DB.prepare('SELECT id, name, totp_secret, totp_enabled FROM staff WHERE id = ?').bind(session.staffId).first<StaffRow>();
