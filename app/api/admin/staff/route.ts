...
export async function POST(request: Request) {
  ...
  // `role` is untrusted request input, still a plain string at this point
  // — the cast below is just so ROLES.includes() (typed StaffRole[]) can
  // check membership; it doesn't assert `role` itself is a StaffRole yet.
  if (!name || !email || !password || !ROLES.includes(role as StaffRole)) {
