...
//
// Not currently imported anywhere in the app (menu content is served from
// D1 via /api/menu — see lib/types.ts's RawCategory/RawProduct) — this
// looks like leftover seed/demo data from before that migration. Left in
// place and converted for completeness rather than deleted, since
// removing unused files is outside this migration's scope; flagged here
// so it's easy to find if it turns out to be genuinely dead.

const img = (q: string, w = 900, h = 700): string =>
...
// Non-null assertion, not optional chaining: the original threw if `id`
// didn't match any category, and `!` preserves that exactly under strict
// mode's stricter Array.prototype.find() typing — `?.` would silently
// change that to a quiet `undefined` instead.
const catImage = (id: string): string => CATEGORIES.find((c) => c.id === id)!.image;
...
].map((it: { id?: string; cat: string; [key: string]: unknown }, i) => ({
  id: it.id || `${it.cat}-${i}`,
