...
export default function BundleModal() {
  ...
  const pickerCategoryIds: string[] = pickerSlot
    ? (pickerSlot.categoryIds && pickerSlot.categoryIds.length
      ? pickerSlot.categoryIds
      : (pickerSlot.categoryId ? [pickerSlot.categoryId] : []))
    : [];
  // p.cat is typed optional (Product.cat?) but always populated for a real
  // menu product — non-null assertion is a no-op fix under strict mode.
  const pickerProducts = pickerSlot ? products.filter((p) => pickerCategoryIds.includes(p.cat!)) : [];

  const pickItem = (product: Product) => {
              ...
              return (
                ...
                >
                  <span className="menu-item-info">
                    ...
                    {/* Same reasoning as pickerProducts above — non-null assertion, no behavior change. */}
